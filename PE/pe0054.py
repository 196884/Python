...
def scoreHand(l):
    # first check if it's a single suit:
    values     = []
    suit       = l[0][1]
    singleSuit = True
    for (v, s) in l:
        values.append(v)
        singleSuit = singleSuit and s == suit
    values.sort()
    isStraight = True
    for i in range(1, 5):
        isStraight = isStraight and values[i] == values[i-1] + 1
    aggVal = aggValue(values)
    if singleSuit:
        # Going for a flush or a straight flush
        if isStraight:
            return 8000000 + aggVal
        return 5000000 + aggVal

    # Not a single suit
    if isStraight:
        return 4000000 + aggVal
    c = combine(values)
    ...
    if nMax == 2:
        ...
        if n2 == 2:
            # Two pairs
            return 2000000 + 15 * 15 * vMax + 15 * n2 + c[0][0]
        # Single pair
        return 1000000 + 15 * 15 * 15 * vMax + 15 * 15 * c[2][0] + 15 * c[1][0] + c[0][0]
    if nMax == 3:
        (v2, n2) = c[0]
        if n2 == 2:
            # Full house
            return 6000000 + 15 * vMax + v2
        # 3 of a kind
        return 3000000 + 15 * 15 * vMax + 15 * c[1][0] + c[0][0]
    # has to be 4 of a kind
    return 7000000 + 15 * vMax + c[0][0]
